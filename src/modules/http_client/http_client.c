...
static int run_queries(void)
{
	...
	if (IS_ENABLED(CONFIG_NET_SOCKETS_SOCKOPT_TLS)) {
		ret = tls_credential_add(CA_CERTIFICATE_TAG,
					 ...
					 http_client_ca_certificate,
					 sizeof(http_client_ca_certificate));
		if (ret < 0) {
	...
	if (IS_ENABLED(CONFIG_NET_IPV4)) {
		ret = connect_socket(AF_INET, SERVER_ADDR4, port,
				     &sock4, (struct sockaddr *)&addr4,
				     sizeof(addr4));
	}

	if (IS_ENABLED(CONFIG_NET_IPV6)) {
		ret = connect_socket(AF_INET6, SERVER_ADDR6, port,
				     &sock6, (struct sockaddr *)&addr6,
				     sizeof(addr6));
	}

	if ((sock4 < 0 && sock6 < 0) || ret < 0) {
